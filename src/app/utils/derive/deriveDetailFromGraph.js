...
function deriveRelationships(node, allEdges) {
  if (!node) return [];

  return allEdges
    .filter((edge) => edge.subject === node.id)
    .map((edge) => ({
...
export function deriveDetailFromGraph(graphProjection, focusNodeId) {
  ...
  return {
    ...
    node: {
      ...
      relationships: deriveRelationships(selectedNode, edges),
